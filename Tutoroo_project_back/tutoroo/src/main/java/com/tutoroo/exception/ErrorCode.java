...
public enum ErrorCode {
    ...
    // [추가] 서비스 로직에서 자주 쓰이는 공통 에러
    ACCESS_DENIED(HttpStatus.FORBIDDEN, "C006", "접근이 거부되었습니다."),
    INVALID_TYPE_VALUE(HttpStatus.BAD_REQUEST, "C007", "유효하지 않은 데이터 타입입니다."),
    MISSING_PARAMETER(HttpStatus.BAD_REQUEST, "C008", "필수 파라미터가 누락되었습니다."),
    RESOURCE_NOT_FOUND(HttpStatus.NOT_FOUND, "C009", "리소스를 찾을 수 없습니다."),
    ...
    INVALID_PASSWORD(HttpStatus.BAD_REQUEST, "U003", "비밀번호가 일치하지 않습니다."), // 400 Bad Request 유지
    LOGIN_REQUIRED(HttpStatus.UNAUTHORIZED, "U004", "로그인이 필요합니다."), // [추가]
    UNAUTHORIZED_ACCESS(HttpStatus.UNAUTHORIZED, "U005", "접근 권한이 없습니다."),
    ...
    SOCIAL_LOGIN_RESTRICTION(HttpStatus.BAD_REQUEST, "U008", "소셜 로그인 유저는 비밀번호를 변경할 수 없습니다."),
    ...
    // [중요 추가] TutorService에서 사용하는 에러 코드
    AI_SERVICE_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "L006", "AI 서비스 연동 중 치명적인 오류가 발생했습니다."),
    ROADMAP_GENERATION_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "L007", "로드맵 생성에 실패했습니다. 다시 시도해주세요."),

    // [호환성] 기존 코드와의 호환을 위해 추가 (STUDY_PLAN_NOT_FOUND와 동일 의미)
    PLAN_NOT_FOUND(HttpStatus.NOT_FOUND, "L002", "학습 계획을 찾을 수 없습니다."),
    ...
    PAYMENT_VERIFICATION_FAILED(HttpStatus.BAD_REQUEST, "M004", "결제 검증에 실패했습니다.");
